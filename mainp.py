...
# -*- coding: utf-8 -*-
"""
Integrated Fake News Detector (simplified, no synonyms.csv, no zipfile)
...
def train_model(fake_csv_path, true_csv_path, model_path='fake_news_model.json'):
    fake_rows = load_news_rows(fake_csv_path, assume_label=0)[:2000]
    true_rows = load_news_rows(true_csv_path, assume_label=1)[:2000]
...
def detect_fake_news(user_news, api_key, model_path='fake_news_model.json', ml_threshold=None):
    """
    Detect if news is REAL or FAKE using ML + NewsAPI verification.
    """
    ...
    # Load optimal threshold from meta
    ...
    if os.path.exists(meta_path):
        try:
            with open(meta_path, 'r', encoding='utf-8') as f:
                meta = json.load(f)
            th_opt = meta.get('optimal_threshold', 0.5)
        except Exception:
            pass
    ...
    ml_verdict = "REAL ✅" if prob >= ml_threshold else "FAKE ❌"

    # Search NewsAPI to see if the headline exists
    articles = fetch_news(api_key, user_news, days_back=30) if api_key else []
    if articles:
        # If matching articles found → declare REAL
        return (
            f"NewsAPI verification found {len(articles)} matching article(s).\n"
            f"Verdict: REAL ✅\n"
            f"ML Probability: {prob:.2f} (threshold={ml_threshold:.2f})"
        )

    # If no articles found → rely on ML
    return (
        f"No corroborating articles found via NewsAPI.\n"
        f"Verdict based on ML: {ml_verdict}\n"
        f"ML Probability: {prob:.2f} (threshold={ml_threshold:.2f})"
    )
...
def download_kaggle_dataset_if_needed(dataset_slug='clmentbisaillon/fake-and-real-news-dataset', zip_name='fake-and-real-news-dataset.zip', expect_files=('Fake.csv', 'True.csv')):
    ...
    logging.info("Dataset downloaded. Please manually extract Fake.csv and True.csv from the zip file.")
...
def main():
    ...
    parser.add_argument('--auto_download', action='store_true', help="Auto-download Kaggle dataset (requires manual extraction)")
    ...
    if args.detect:
        ...
        result = detect_fake_news(user_news=args.detect, api_key=args.api_key, model_path=args.model_path, ml_threshold=args.ml_threshold)
        print(result)
